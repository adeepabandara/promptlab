...
export function useConceptRuns(filters?: {
  modelId?: string;
  promptId?: string;
  status?: string;
}) {
  ...
  const fetch = useCallback(async () => {
    ...
    const newRuns = (data as ConceptRun[]) ?? [];
    ...
    setRuns(newRuns);
    setLoading(false);
  }, [filters?.modelId, filters?.status]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('concept_runs_changes')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'concept_runs' },
        () => fetch()
      )
      .subscribe();
  ...
  // Polling fallback: keep refreshing while any run is in-progress
  useEffect(() => {
