...
interface ImageRunListProps {
  runs: ImageRun[];
  loading: boolean;
  selectedId?: string;
  onSelect: (run: ImageRun) => void;
}

export function ImageRunList({ runs, loading, selectedId, onSelect }: ImageRunListProps) {
  if (loading) {
    return (
              ...
              >
                ...
                <div className="flex items-center justify-between mt-1">
                  <span className="text-xs text-gray-400">
                    ...
                    {(run.output_image_urls ?? []).length > 0 && ` · ${(run.output_image_urls ?? []).length} img`}
                  </span>
                  {run.quality_rating && <StarRating value={run.quality_rating} readOnly />}
                </div>
              </button>
            ))
          )}
